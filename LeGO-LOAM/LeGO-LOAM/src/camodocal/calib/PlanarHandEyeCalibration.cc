...
PlanarHandEyeCalibration::PlanarHandEyeCalibration()
 : m_verbose(false), m_refine(false)
{
...
void
PlanarHandEyeCalibration::setRefine(bool on)
{
    m_refine = on;
}
                                   ...
                                   Eigen::Matrix4d& H_12) const
{
	...
	std::cout << "R_yx: " << std::endl << R_yx << std::endl;
	
    // Estimate R_z, t_x, t_y, using linear least square Gm=w
    ...
    if (m_verbose)
    {
        std::cout << "# INFO: Before refinement: initialization" << std::endl;
    ...
    if ((m_verbose) && (m_refine))
    {
        std::cout << "# INFO: After refinement: non-linear optimization" << std::endl;
        std::cout << "H_12 = " << std::endl;
        std::cout << H_12 << std::endl;
    }
