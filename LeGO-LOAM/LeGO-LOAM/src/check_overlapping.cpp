...
// 		std::cout << index_overlap << std::endl;
		...
		ss << "/home/jjiao/catkin_ws/src/localization/LeGO-LOAM/LeGO-LOAM/data/yellow_20190104/8_shape/evaluation/pose_graph_" << i_ref << ".txt";
		std::string filename = ss.str();
		ss.str("");
		int pose_id = 2;
		
		std::ofstream ofs(filename.c_str(), std::ios::out | std::ios::binary);
		ofs << "pose_id x y z q_x q_y q_z q_w" << std::endl;
		ofs << "1 0 0 0 0 0 0 1" << std::endl;
		
		std::vector<float> v_edge;
		matrix_to_vector(v_tf_ini[1], v_edge);
		ofs << pose_id << " " << v_edge[0] << " " << v_edge[1] << " " << v_edge[2] 
			<< " " << v_edge[4] << " " << v_edge[5] << " " << v_edge[6] << " " << v_edge[3] << std::endl;
		pose_id ++;
		...
		if (b_bag)
		{
			...
			for (size_t j = i_start; j <= i_end; j++)
			{
				...
				std::vector<float> v_edge;
				matrix_to_vector(data_lidar_cloud.ini_.block<4,4>(0,0), v_edge);
				ofs << pose_id << " " << v_edge[0] << " " << v_edge[1] << " " << v_edge[2] << " " << v_edge[4] 
						<< " " << v_edge[5] << " " << v_edge[6] << " " << v_edge[3] << std::endl;
				pose_id ++;
