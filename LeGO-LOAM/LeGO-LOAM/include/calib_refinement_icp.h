#ifndef CALIB_REFINEMENT_ICP_H_
#define CALIB_REFINEMENT_ICP_H_

#include "visualizer/Visualizer.h"
#include "lidar_data.h"
#include "utility.h"

class CalibRefinementICP
{
public:
	CalibRefinementICP(ros::NodeHandle &nh) : nh_(nh)
	{}
	...
	void systemInitialization()
	{
		...
		refLidarMap.systemInitialization();
		dataLidarMap.systemInitialization();
...
// 		frameCount = skipFrameNum;
	...
	void setData(LidarMap &ref, LidarMap &data, const Eigen::Matrix4f &tf)
	{
		refLidarMap.setLidarMap(*ref.cornerCloud_, *ref.surfCloud_, *ref.fullCloud_, ref.header_);
		dataLidarMap.setLidarMap(*data.cornerCloud_, *data.surfCloud_, *data.fullCloud_, data.header_);
		ini_tf.matrix() = tf;
	}
...
// 		printResult();
	...
	{
		kdtreeCornerRef->setInputCloud(refLidarMap.cornerCloud_);
		kdtreeSurfRef->setInputCloud(refLidarMap.surfCloud_);
		numCornerRef = refLidarMap.cornerCloud_->size();
		numSurfRef = refLidarMap.surfCloud_->size();
		for (int i = 0; i < 6; ++i)
		{
				transformCur[i] = 0;
				transformSum[i] = 0;
	...
	void updateTransformation()
	{
		...
		downSizeFilterSurf.setInputCloud(dataLidarMap.surfCloud_);
		downSizeFilterSurf.filter(*dataLidarMap.surfCloud_);
		downSizeFilterCorner.setInputCloud(dataLidarMap.cornerCloud_);
		downSizeFilterCorner.filter(*dataLidarMap.cornerCloud_);		
		
		std::cout << "Number of ref corner and surf: " << numCornerRef << " " << numSurfRef << std::endl;
		std::cout << "Number of data corner and surf: " << dataLidarMap.cornerCloud_->size() 
			<< " " << dataLidarMap.surfCloud_->size() << std::endl;
		...
		for (int iterCount1 = 0; iterCount1 < 25; iterCount1++) 
		{
			...
			plane_normal_visualizer.UpdateLines(refLidarMap.surfCloud_, dataLidarMap.surfCloud_, 
												surfPointsCorr[0], surfPointsCorr[1]);
			...
			std::cout << "iterCount: " << iterCount1 << ": number of coorespondences: " 
...
// 	
// 			// visualize correspondence
// 			plane_normal_visualizer.UpdateLines(refLidarMap.cornerCloud_, dataLidarMap.cornerCloud_, 
...
// 		temp_tf.translate(Eigen::Vector3f(transformSum[3], transformSum[4], transformSum[5]));
	...
	{
		int cornerPointsSharpNum = dataLidarMap.cornerCloud_->points.size();
		for (int i = 0; i < cornerPointsSharpNum; i++) 
		{
			TransformToStart(&dataLidarMap.cornerCloud_->points[i], &pointSel);
			if (iterCount % 5 == 0) 
			{
				...
				if (pointSearchSqDis[0] < nearestFeatureSearchSqDist) 
				{
					...
					int closestPointScan = int(refLidarMap.cornerCloud_->points[closestPointInd].intensity);
					float pointSqDis, minPointSqDis2 = nearestFeatureSearchSqDist;
					for (int j = closestPointInd + 1; j < cornerPointsSharpNum; j++) 
					{
						if (int(refLidarMap.cornerCloud_->points[j].intensity) > closestPointScan + 2.5) 
						{
							break;
						}
	
						pointSqDis = (refLidarMap.cornerCloud_->points[j].x - pointSel.x) * 
										(refLidarMap.cornerCloud_->points[j].x - pointSel.x) + 
										(refLidarMap.cornerCloud_->points[j].y - pointSel.y) * 
										(refLidarMap.cornerCloud_->points[j].y - pointSel.y) + 
										(refLidarMap.cornerCloud_->points[j].z - pointSel.z) * 
										(refLidarMap.cornerCloud_->points[j].z - pointSel.z);
	
						if (int(refLidarMap.cornerCloud_->points[j].intensity) > closestPointScan) 
						{
			...
			{	
				tripod1 = refLidarMap.cornerCloud_->points[pointSearchCornerInd1[i]];
				tripod2 = refLidarMap.cornerCloud_->points[pointSearchCornerInd2[i]];
...
// 				if (s > 0.1 && ld2 != 0) 
				if (ld2 != 0)
				{
					...
					laserCloudOri->push_back(dataLidarMap.cornerCloud_->points[i]);
					coeffSel->push_back(coeff);
					CornerPointsCorr[0]->push_back(refLidarMap.cornerCloud_->points[pointSearchCornerInd1[i]]);
					CornerPointsCorr[1]->push_back(dataLidarMap.cornerCloud_->points[i]);					
				}
			}
		}
	}
...
// 		int surfPointsFlatNum = dataLidarData.surfPointsFlat->points.size();
		int surfPointsFlatNum = dataLidarMap.surfCloud_->points.size();		
	
		for (int i = 0; i < surfPointsFlatNum; i++) 
		{
// 			TransformToStart(&dataLidarData.surfPointsFlat->points[i], &pointSel);
			TransformToStart(&dataLidarMap.surfCloud_->points[i], &pointSel);			
			if (iterCount % 5 == 0) 
			{
				// find closest neighborhood
				...
				{
					closestPointInd = pointSearchInd[0];
					int closestPointScan = int(refLidarMap.surfCloud_->points[closestPointInd].intensity);
	
					float pointSqDis, minPointSqDis2 = nearestFeatureSearchSqDist, minPointSqDis3 = nearestFeatureSearchSqDist;
					for (int j = closestPointInd + 1; j < surfPointsFlatNum; j++) 
					{
						if (int(refLidarMap.surfCloud_->points[j].intensity) > closestPointScan + 2.5) 
						{
							break;
						}
	
						pointSqDis = (refLidarMap.surfCloud_->points[j].x - pointSel.x) * 
										(refLidarMap.surfCloud_->points[j].x - pointSel.x) + 
										(refLidarMap.surfCloud_->points[j].y - pointSel.y) * 
										(refLidarMap.surfCloud_->points[j].y - pointSel.y) + 
										(refLidarMap.surfCloud_->points[j].z - pointSel.z) * 
										(refLidarMap.surfCloud_->points[j].z - pointSel.z);
	
						if (int(refLidarMap.surfCloud_->points[j].intensity) <= closestPointScan) 
						{
							if (pointSqDis < minPointSqDis2) 
							{
								minPointSqDis2 = pointSqDis;
			...
			if (pointSearchSurfInd2[i] >= 0 && pointSearchSurfInd3[i] >= 0) 
			{
				...
				tripod1 = refLidarMap.surfCloud_->points[pointSearchSurfInd1[i]];
				tripod2 = refLidarMap.surfCloud_->points[pointSearchSurfInd2[i]];
				tripod3 = refLidarMap.surfCloud_->points[pointSearchSurfInd3[i]];
...
// 					laserCloudOri->push_back(dataLidarData.surfPointsFlat->points[i]);
					laserCloudOri->push_back(dataLidarMap.surfCloud_->points[i]);
					coeffSel->push_back(coeff);
					surfPointsCorr[0]->push_back(refLidarMap.surfCloud_->points[pointSearchSurfInd1[i]]);
					surfPointsCorr[1]->push_back(dataLidarMap.surfCloud_->points[i]);
				}
			}
		}
	}
...
// 		pcl::transformPointCloud(*dataLidarMap.cornerPointsLessSharp, cornerPointsLessSharpTransformed, estimated_tf);
// 		common::publishCloud(pubDataCornerPointsLessSharpTransformed, dataLidarMap.header_, cornerPointsLessSharpTransformed);		
// 		
// 		pcl::PointCloud<PointType> surfPointsLessFlatTransformed;
// 		pcl::transformPointCloud(*dataLidarMap.surfPointsLessFlat, surfPointsLessFlatTransformed, estimated_tf);
// 		common::publishCloud(pubDataSurfPointsLessFlatTransformed, dataLidarMap.header_, surfPointsLessFlatTransformed);
...
// 		tfBroadcaster.sendTransform(laserOdometryTrans);
	...
	LidarMap refLidarMap, dataLidarMap;
